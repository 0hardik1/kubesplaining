...
8. **privesc** — **the differentiator.** Builds a directed graph where nodes are RBAC subjects and sinks like `cluster-admin`, `node-escape`, `kube-system-secrets`, `system:masters`. Edges are labeled with the technique that enables the hop. Runs BFS from every non-system subject (capped at `--max-privesc-depth`, default 5) and emits one finding per `(source, sink)` pair with the full hop-by-hop chain attached. A second pass then re-runs the search per source with the binding that granted the first hop banned; a route that still reaches the sink becomes the finding's `AlternateEscalationPath` plus the tag `privesc:survives-first-cut`, so the finding states whether its own recommended fix actually closes the route. Edges also declare the foothold they need at their source and the foothold they give at their destination (`models.Foothold`: acting as the subject's API identity, holding a presentable token for it, a shell in a ServiceAccount's existing pods, or a new pod created as it). BFS walks an edge only when the attacker holds a foothold it needs, so a minted token never reaches an existing pod's host escape; exec into a pod that mounts no API token gives the pod but not the ServiceAccount's RBAC; and impersonating a ServiceAccount, which acts as it but mints no token, does not reach its IRSA role. Exec edges also honor `resourceNames`, reaching only the pods a scoped grant can name.
