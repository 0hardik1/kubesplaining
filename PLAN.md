...
**CIS/NSA framework tags on every rule** (STRATEGY.md Tier 2.5). Add a `Frameworks []FrameworkRef` field to `models.Finding`, a static rule → control-ID mapping table for CIS Kubernetes Benchmark v1.9 and NSA/CISA Kubernetes Hardening Guide, and a "Compliance Coverage" tab in the HTML report. Enterprise evaluation gate: scanners that don't tag findings with framework control IDs don't get adopted. The field is additive and back-compatible with existing JSON/SARIF consumers. After that, the next-largest leverage item is **`Finding.Remediation.Patch`** (a generated `kubectl patch` or Kyverno policy alongside each finding) — STRATEGY.md Tier 2.6.
