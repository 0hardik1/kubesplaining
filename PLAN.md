...
- [x] Cloud identity edges (IRSA + aws-auth + IMDS-pivot for EKS), implemented in [internal/analyzer/privesc/cloud_edges.go](internal/analyzer/privesc/cloud_edges.go). Adds three edge shapes: (1) IRSA edges from SA to an external `aws-iam` node; (2) aws-auth edges from the external node onward to `system_masters` / `cluster_admin` sinks; (3) IMDS-pivot edges from SA to `node_escape` when the pod can reach 169.254.169.254 and lacks an IRSA binding. External nodes carry `IsExternal=true` and are skipped as BFS sources. Note: `KUBE-PRIVESC-PATH-AWS-IAM-ROLE` is wired in [analyzer.go](internal/analyzer/privesc/analyzer.go) but currently never emits because the pathfinder only terminates at `IsSink` nodes; a follow-up can promote no-outbound-edge external nodes to sinks.
